// ===== IMPORTS =====                                     const TelegramBot = require('node-telegram-bot-api');
...
const ADMIN_ID = 7577278314;
                                                           const addresses = {
  BEP20: "0x7fc952f9c38facc2a46fe1d863267d01dda7276d",       TRC20: "TVCvtgXAjuHGkJQ6FK5sLMDCuA72MLdz3n",
  TON: "UQCk6ZT-Xmi8-Hk2JyEUXhM8j1n0ufxp-UmXZ_F1OKhLLjqy"
};                                                         
// ===== MONGODB SETUP =====                               mongoose.connect(process.env.MONGO_URI)                      .then(() => console.log("✅ MongoDB Connected"))           .catch(err => console.log("❌ MongoDB Error:", err));    
// ===== SCHEMAS =====
const userSchema = new mongoose.Schema({
  ...
  method: String,
  amount: Number,
  details: String,                                           orderId: Number,
  isDemo: Boolean,                                           demoUsed: Boolean,
  replyTo: String
});
                                                           const orderSchema = new mongoose.Schema({
  orderId: Number,                                           processed: Boolean
});
...
// ===== HELPERS =====
async function getUser(chatId, username = "NoUsername", name = "User") {                                                const user = await User.findOneAndUpdate(
    { chatId },                                                { $setOnInsert: { username, name, demoUsed: false } },
...
// ---------------- START MENU ----------------            async function showMainMenu(chatId) {
...
See it happen for yourself — the way it looks might surprise you.                                                                                                                📸 Some users even test different things with it… 👀
...
bot.onText(/\/start/, (msg) => {
  showMainMenu(msg.chat.id);                               });                                                                                                                   // ---------------- CALLBACK ----------------              bot.on("callback_query", async (query) => {
...
Send $20 or more:`,
      {
        ...
        reply_markup: {                                              inline_keyboard: [
            ...
            [{ text: "🔺 TRC20", callback_data: "pay_TRC20" }],                                                                   [{ text: "🔷 TON", callback_data: "pay_TON" }],            [{ text: "🔙 Back", callback_data: "back" }]
          ]
        }
      }
    );
    ...
    bot.editMessageText(                                   `💎 Flash USDT Price List
...
💵 $100 Real USDT → $6000 Flash Balance
  ...
  } else if (data === "support") {                             bot.editMessageText(
`📩 Support Center                                         
Choose option:`,                                                 {
        chat_id: chatId,
        message_id: query.message.message_id,
      ...
      }                                                        );                                                                                                                  } else if (data === "demo") {                                if (user.demoUsed) return bot.answerCallbackQuery(query.id, { text: "❌ Demo already used" });                        user.isDemo = true;
    await user.save();                                     
    bot.editMessageText(                                   `🎁 Demo Plan >> Once per user
...
Send your message here and our admin will reply to you inside the bot.
    ...
    bot.editMessageText(
`${method} Payment                                         
Send ${amountText} to:                                     \`${addresses[method]}\`                                                                                              After payment, click below:`,
      {
        parse_mode: "Markdown",
    ...
    user.step = "details";                                     await user.save();                                                                                                    bot.sendMessage(                                             chatId,                                              `📩 Send TX# & receiving address:                          
TXID  - 0x2d3d7abb690bbc65a45cea897667a3bea80bd55bd309517e189b7458bab74d03
BEP20 - 0x8f3a0000000000000000000000000000000000a1

Make sure the address belongs to your wallet.
Trust Wallet or Binance Web3.`
    ...
    if (targetUser.isDemo) {                                     targetUser.demoUsed = true;                                await targetUser.save();                                 }
    ...
    if (order && order.processed) {
      const targetUser = await User.findOne({ orderId });        bot.sendMessage(targetUser.chatId, "❌ Payment not found. If you believe it's a mistake, reach out to support");      return bot.answerCallbackQuery(query.id, { text: "Already processed" });
    }
...
bot.on("message", async (msg) => {
  ...
  if (user.step === "admin_reply") {
    ...
    bot.sendMessage(chatId, "✅ Reply sent");                  user.step = null;                                          await user.save();
    return;                                                  }
                                                             if (user.step === "support") {                               bot.sendMessage(                                             ADMIN_ID,                                            `Support from ${chatId} (${name} ${username}):             ${text}`,
      {                                                            reply_markup: {                                              inline_keyboard: [
  ...
  if (user.step === "details") {                               user.details = text;                                       user.step = "amount";
    await user.save();                                         bot.sendMessage(chatId, "💰 Enter amount sent:");          return;                                                  }                                                                                                                     if (user.step === "amount") {
    ...
    user.amount = amount;
    user.orderId = Date.now() + Math.floor(Math.random() * 1000);                                                         user.step = null;                                          await user.save();                                     
    bot.sendMessage(
      chatId,                                              `⏳ Processing your request...
...
`New Order                                                 User: ${chatId} (${name} ${username})
Amount: $${amount}
Method: ${user.method}
...
bot.onText(/\/broadcast (.+)/, async (msg, match) => {       if (msg.chat.id != ADMIN_ID) return;
